(function() {
    ...
    function handleSelection(event) {
        ...
        if (selectedText.length >= 2 && selection.rangeCount > 0) {
            ...
            showIconTimeout = setTimeout(() => {
                if (lastMouseUpEvent) {
                    const iconX = lastMouseUpEvent.pageX + 15;
                    const iconY = lastMouseUpEvent.pageY + 15;
    ...
    function hideIconOnActivity() {
        if (floatingIcon && floatingIcon.style.display !== "none") {
            hideFloatingIcon();
        }
    }
    document.addEventListener("scroll", hideIconOnActivity, true);
    document.addEventListener("keydown", hideIconOnActivity, true);
