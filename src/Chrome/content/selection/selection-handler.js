(function() {
    (async () => {
    try {
      const storageApi = typeof chrome !== 'undefined' ? chrome : null;
      if (storageApi && storageApi.storage && storageApi.storage.local) {
  ...
  function createFloatingIcon() {
    ...
    try {
      iconImage.src = chrome.runtime.getURL("../../icons/icon-128.png");
    } catch (e) {
      iconImage.src = "";
    }
  ...
  function createTranslatePopup() {
    ...
    document.getElementById("tangoji-popup-save").addEventListener("click", () => {
      if (currentSelectedText) {
        chrome.runtime.sendMessage({ action: "saveToTangojiViaPopup", text: currentSelectedText }, (response) => {
          if (response && response.success) {
            console.log("Text Popup: Save successful", response.message);
          } else {
  ...
  function showTranslatePopup(text, baseRect) {
    ...
    chrome.runtime.sendMessage({ action: "getAiTranslation", text: text }, (response) => {
